...
## Nouveau système de déploiement depuis la version 9.25.0 (beta)

À partir de la version 9.25.0, un nouveau système de déploiement est disponible en version beta. Il permet de créer des déploiements depuis la console afin d'associer un environnement à un ou plusieurs dépôts, chacun avec sa propre configuration (révision, chemin, sources de valeurs internes ou externes).

Pour l'activation des droits, la création et la configuration des déploiements, voir la page dédiée : [Gestion des déploiements](/guide/deployment-management).
...
Si un `drift` est identifé :
 ...
 > :exclamation: Il est nécessaire de corriger tous les `drifts` avant de passer l'environnement en `Synchronisation automatique` afin de ne pas impacter vos applications.
  ...
  Afin de corriger les `drifts` plusieurs cas sont possibles :
   ...
   > :white_check_mark: Depuis la version 9.25.0, ce cas peut être géré avec le nouveau système de déploiement en version beta.
