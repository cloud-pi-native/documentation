...
## Cumul et "priorité" des rôles

Les permissions sont cumulatives : un utilisateur obtient l’ensemble des permissions présentes dans tous les rôles qui lui sont assignés. Il n’y a pas de mécanisme de “droit le plus faible” ou de “refus” qui viendrait retirer des droits.

En pratique, si un utilisateur a plusieurs rôles (par exemple `Administrateur`, `DevOps` et `Développeur`), ses permissions effectives correspondent au niveau le plus élevé atteint sur chaque action, puisque les permissions s’ajoutent.
